...
2. 定义FM算法；
3. 损失函数梯度的定义；loss = self.sigmoid(classLabels[x] * p[0, 0]) -1
4. 利用梯度下降更新参数。gradient = (self.sigmoid(classLabels[x] * p[0, 0]) -1)*classLabels[x]*p_derivative
下面的代码片段是以上四点的描述，其中的loss并不是二分类的损失loss，而是分类loss的梯度中的一部分：
'''
代码参考
https://github.com/Chaucergit/Code-and-Algorithm/blob/master/ML/3.Factorization%20Machine/Train/Train.py
...
'''
     data : 数据的路径 feature_potenital : 潜在分解维度数  alpha ： 学习速率
    _w,_w_0,_v ： 拆分子矩阵的weight
    with_col : 是否带有columns_name   first_col : 首列有价值的feature的index
'''
...
class fm(object):
    ...
    #读取数据label和matrix
    def loadDataSet(self, data, with_col=False, first_col=2):
